...
> [!IMPORTANT]
> **Эта страница описывает версию 1.0.0**, а проект с тех пор ушёл вперёд. Всё, что
> добавлено, изменено и исправлено после неё — защита от просадок FPS, раздел целевого
> окна, текстовый расширитель, краш в редакторе и поиск шаблона, который работал в семь
> раз медленнее, чем мог, — записано в **[CHANGELOG_RU.md](CHANGELOG_RU.md)**
> ([in English](CHANGELOG.md)). Прочитайте его прежде, чем решать, что эта программа умеет.
>
> Две возможности достаточно велики, чтобы иметь собственные руководства, и оба написаны
> для человека, который никогда этим не занимался:
>
> - **[SCRIPTS_RU.md](SCRIPTS_RU.md)** — как заставить макрос ждать, решать и повторять
>   вместо слепого воспроизведения.
> - **[EXPANDER_RU.md](EXPANDER_RU.md)** — как короткое сокращение превращается в длинный
>   текст прямо при наборе.
...
| | |
|---|---|
| 🧠 **Движок скриптов** | 17 видов шагов с `Если` / `Пока` / `Прервать`, переменные и условия, которые смотрят на экран. **[Полное руководство → SCRIPTS_RU.md](SCRIPTS_RU.md)** |
| 🔎 **Поиск по картинке** | Вырезал кусок экрана через `Win+Shift+S` — и макрос ждёт эту картинку или кликает по ней, где бы она ни появилась |
| 🔤 **Текст на экране (OCR)** | Использует распознавание, уже встроенное в Windows: реагируй на слова или считывай число в переменную |
...
| **Скрипты / условная логика** | ❌ | ✅ **17 видов шагов, `Если`/`Пока`/`Прервать`, переменные** — [SCRIPTS_RU.md](SCRIPTS_RU.md) |
...
**17 видов шагов:** `Воспроизвести события` · `Пауза` · `Ждать` · `Клик по картинке` · `Клик в` · `Клавиша` · `Присвоить` · `Если` · `Иначе` · `Конец если` · `Пока` · `Конец пока` · `Прервать` · `Запустить` · `Закрыть программу` · `Заметка` · `Прочитать число`

**6 условий:** `всегда` · `переменная` · `картинка` · `пиксель` · `окно` · `текст`
...
| **Искать только в области** | Ограничивает поиск прямоугольником, что заметно быстрее на 4K-экране |

> ⚠️ **Обе опции работают только для кнопки проверки.** Внутри скрипта шаги с картинками всегда сканируют **весь виртуальный рабочий стол в масштабе 1.0×**. Если нужен быстрый поиск в скрипте — берите шаблон поменьше.
...
| **Скриптовый поиск картинки — по всему экрану** | Опции «искать только в области» и «другие масштабы» относятся только к панели проверки; шаги скрипта всегда сканируют весь стол в масштабе 1.0× |
