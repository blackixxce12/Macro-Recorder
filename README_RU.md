...
| **Хоткеи не попадают в запись** | ✅ по устройству программы | ✅ |
...
- ⌨ Переназначаемые глобальные хоткеи: запись, воспроизведение и отдельный **аварийный стоп** (по умолчанию `F6` / `F7` / `F8` / `F9`)
...
| Пауза / продолжение | `F8` | Переназначается, либо кнопка в окне |
...
Нажмите кнопку с клавишей и затем любую клавишу — буквы, цифры, функциональные. На время привязки
глобальные хоткеи снимаются, поэтому `F6` и `F7` можно поменять местами; Esc или 15 секунд без
нажатий отменяют. Список ▾ рядом содержит клавиши, которые окно не получает вовсе: `Pause`,
`ScrollLock`, NumPad. **«Сбросить»** снимает привязку полностью.
...
Макрос хранит абсолютные экранные координаты, поэтому обычно он ломается, стоит передвинуть
целевое окно. Привязка это чинит.

Включите «Запоминать целевое окно» (по умолчанию выключено) — и в момент старта записи приложение
запомнит заголовок и позицию окна, которое было на переднем плане: игры, браузера, чего угодно, по
чему вы собирались кликать. Эта пара сохраняется внутрь файла макроса.

Позже, если включить «Следовать за окном привязки», воспроизведение найдёт это окно по заголовку,
измерит, насколько оно сместилось с момента записи, и сдвинет на столько же все координаты.
Перетащите окно в другую половину экрана — макрос всё равно попадёт по тем же кнопкам. Если окна нет,
воспроизведение идёт без сдвига и пишет об этом в лог.

По умолчанию выключено, потому что заголовок окна попадает внутрь файла макроса, и потому что макрос,
записанный сразу по нескольким окнам (или прямо по рабочему столу), лучше оставить без привязки.
...
| `record_window_anchor` | bool | `false` | Запоминать окно переднего плана при старте записи |
...
| `hotkey_record` / `hotkey_play` / `hotkey_stop` / `hotkey_pause` | объект | F6 / F7 / F9 / F8 | `{ "vk": 117, "ctrl": false, "alt": false, "shift": false }`; `vk: 0` — не назначено |
...
Issues и PR приветствуются. Если сообщаете о баге воспроизведения, приложите файл макроса (можно обрезанный), нужный кусок `logs/macro-recorder.log.*`, версию Windows, масштаб экрана и раскладку мониторов.
